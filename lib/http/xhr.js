 ...
 */
 AWS.XHRClient = AWS.util.inherit({
  handleRequest: function handleRequest(httpRequest, httpOptions, callback, errCallback) {
    ...
    if (['GET', 'HEAD', 'OPTIONS'].includes(httpRequest.method)) {
      if (httpRequest.body && httpRequest.body.length) {
        href += '?' + httpRequest.body;
      }
    ...
    fetch(request).then(function(response) {
      ...
      response.text().then(function(res) {
        ...
        errCallback(AWS.util.error(new Error('XML Parsing Failure'), {
          code: 'InternalError'
        }));
      });
